<?php
    ...
    if (isset($_SESSION['user']['idRol'])) {
        ...
        if($idSugerenciaSeleccionada<0) {
            ...
            $sqlCantidad = $pdo->prepare("SELECT COUNT(idOpinion) as cantidad FROM opiniones WHERE idOpinion=?");
            $sqlCantidad->execute([$idSugerenciaSeleccionada]);
            $sqlCantidad = $sqlCantidad->fetch(PDO::FETCH_ASSOC);
    ...
    <script src="JavaScript/mod-Sugerencias.js"></script>
<?php 
    }else {
