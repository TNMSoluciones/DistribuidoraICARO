            ...
            if($_POST['accion'] == 'insertar') {
                ...
                if($sqlConsultaProducto==0) {
                    ...
                    if($extension=='image/jpeg' || $extension=='image/jpg' || $extension=='image/png') {
                        ...
                        $sqlInsert=$pdo->prepare("INSERT INTO producto(Nombre, Stock, Precio, Imagen, idCategoria, idPersonal) VALUES(?,?,?,?,?,?)");
                        $sqlInsert->execute([$nameProduct,$stockProduct,$priceProduct,$binariosImg,$catProduct,$empleadoMod]);
                        fclose($imgSubida);
                        echo $sqlInsert? 1 : 2; 
                    }
                }else{echo 3;}
            ...
            if($_POST['accion'] == 'actualizar') {
                ...
                if($sqlConsultaProducto=1) {   
                    ...
                    if($_FILES['imgProduct']['name']=='') {
                        $sqlUpdate=$pdo->prepare('UPDATE producto SET Nombre=?, Stock=?,Precio=?, Destacado=?,idCategoria=?,idPersonal=? WHERE idProducto=?');
                        $sqlUpdate->execute([$nameProduct,$stockProduct,$priceProduct,$desProduct,$catProduct,$empleadoMod,$idProduct]);
                        echo $sqlUpdate?1:2;
                    }else{
                        $extension = $_FILES['imgProduct']['type'];
                        if($extension=='image/jpeg' || $extension=='image/jpg' || $extension=='image/png') {
