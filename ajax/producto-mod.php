                ...
                if($extension=='jpeg' || $extension=='jpg' || $extension=='png')
                {
                    ...
                    if(!$bandera){
                        if(move_uploaded_file($_FILES['imgProduct']['tmp_name'], '../imgProductos/'.$nameProduct.'.'.strtolower(pathinfo($_FILES['imgProduct']['name'], PATHINFO_EXTENSION)))){
                            ...
                            $sqlInsert=$pdo->prepare("INSERT INTO producto(Nombre, Stock, Precio, Imagen, idCategoria, idPersonal) VALUES('$nameProduct','$stockProduct','$priceProduct','$urlImagen','$catProduct', 8)");
                            $sqlInsert->execute();
                            echo $sqlInsert? 1 : 2; 
