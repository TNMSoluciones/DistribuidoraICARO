<?php
    ...
    if (isset($data)) {
        ...
        $sqlLoginClientes=$pdo->prepare("SELECT COUNT(RUT) FROM cliente WHERE CorreoCliente=?");
        $sqlLoginClientes->execute([$email]);
        $sqlLoginClientes = $sqlLoginClientes->fetch(PDO::FETCH_ASSOC);
        $sqlLoginClientes=$sqlLoginClientes['COUNT(RUT)'];
        if ($sqlLoginClientes==1) {
            $sqlClientes=$pdo->prepare("SELECT NombreEmpresa, CorreoCliente, RUT, Activo, Password FROM cliente WHERE CorreoCliente=?");
            $sqlClientes->execute([$email]);
            $usuario=$sqlClientes->fetch(PDO::FETCH_ASSOC);
            if ($sqlClientes->rowCount() == 1) {
